...
//                    Log.d("getWishListVMerror", response.body()?.errors.toString())
    ...
    fun getPastBooking (pageSize: Int, pageNumber: Int, authToken: String){
        viewModelScope.launch {
            try {
                val response = customerRepository.getCustomerBookingsByUserId(pageSize, pageNumber, authToken)
                if (response.isSuccessful){
